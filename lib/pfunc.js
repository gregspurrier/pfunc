...
function create() {
  ...
  // downstream promise.
  function invokeCascadeCallback(cb, nextPromise, args) {
    try {
      var result = cb.apply(null, args);
      if (isPromise(result)) {
        result.then(nextPromise.fulfill, nextPromise.reject);
      } else {
  ...
  function cascader(cb, passThrough, nextPromise) {
    if (cb instanceof Function) {
      return invokeCascadeCallback.bind(null, cb, nextPromise);
    } else {
